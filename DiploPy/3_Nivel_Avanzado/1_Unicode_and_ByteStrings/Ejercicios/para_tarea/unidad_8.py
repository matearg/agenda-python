...
class Producto:
    ...
    def __init__(self, window):
        ...
        ttk.Button(self.root, text='Mostrar registros existentes',
                   command=lambda: self.mostrar()).grid(row=5,
                                                        columnspan=3,
                                                        sticky=W + E
                                                        )

        Button(self.root, text="Crear bd",
               command=lambda: base_datos.crearbd()).grid(row=6, column=0)
        Button(self.root, text="Alta", command=lambda: self.alta()).grid(
            row=6, column=1)

        Button(self.root, text='Guardar',
               command=lambda: self.pasar_objeto_guardar()).grid(row=11, column=0)
